...
namespace TAP.UPDATER.Engine {
    ...
    class PatcherEngine : IPatcherEngine {
        ...
        private static void DownloadContent(int step, List<string> content, bool isMissingContent) {
            ...
            Debug.WriteLine("Start - download and write a list of specific files");
            for (int i = 0; i < content.Count; i++) {
                Debug.WriteLine(string.Format("{0} of {1} - Path : {2}, Resource : {3}, ServerMetaData : {4}", i.ToString(), content.Count.ToString(), content[i], PatchDirectory + content[i], ServerMetadata[content[i]].ToString()));
            ...
            Debug.WriteLine("End - download and write a list of specific files");
        ...
        private static void GenerateServerMetadata(int step) {
            ...
            Debug.WriteLine("Start - server's metadata file into memory");
            Debug.WriteLine(string.Format("EngineConfigs.PATCH_METADATA - server's metadata file into memory", EngineConfigs.PATCH_METADATA));
            ...
            Debug.WriteLine("End - server's metadata file into memory");
