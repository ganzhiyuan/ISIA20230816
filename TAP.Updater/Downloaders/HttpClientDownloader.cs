...
namespace TAP.UPDATER.Downloaders {
    /// <summary>
    ...
    public class HttpClientDownloader : IDownloader {
        ...
        private static byte[] DownloadData(string address, string expectedHash, string filePath = null) {
            ...
            while (true) {
                try {
                    Debug.WriteLine(string.Format("Address : {0}, expectedHash : {1}, FilePath : {2}", address,expectedHash,filePath));
        ...
        private static void SetupDefaultHeaders() {
            HttpClient.DefaultRequestHeaders.Add("Accept", "text/plain,application/xhtml+xml,application/xml,image/png,image/jpeg,application/x-msdownload,image/svg+xml,application/x-msaccess,image/bmp,application/json,application/octet-stream,text/xml,application/vnd.ms-access,image/x-icon,application/x-ms-manifest,application/x-ms-application");
