    ...
    class DetectingService : TAP.WinService.ServiceBase
    {
        ...
        private void SendMail()
        {
            ...
            */
            ...
            tmpmaillist.Add(_mailAddress);

            //if (tmpmaillist.Count > 0 && mailData.Rows.Count > 0)
            if (tmpmaillist.Count > 0)
            {
                Mail mail = new TAP.Base.Communication.Mail();
                mail.SendMail("SPC RULE OUT DETECTING DATA - " + _measureDate, "DETECTING_MAIL_SERVICE", messageBody, tmpmaillist);
            }
