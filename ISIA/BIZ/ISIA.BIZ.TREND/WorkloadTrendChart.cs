 ...
 ORDER BY dbid, snap_id");
        ...
        public void GetWorkLoadLagestDaySql(AwrArgsPack arguments)
        {
            ...
            try
            {
                ...
                {
                    tmpSql.AppendFormat("select sql_id,ROUND(sum({0})/8192,0) physical_write_block,instance_number,b.module,b.action,b.parsing_schema_name from (", arguments.ParamNamesString);
                }
                else
                {
                    tmpSql.AppendFormat("select sql_id,ROUND(sum({0}),0) {0},instance_number,b.module,b.action,b.parsing_schema_name from (", arguments.ParamNamesString);
