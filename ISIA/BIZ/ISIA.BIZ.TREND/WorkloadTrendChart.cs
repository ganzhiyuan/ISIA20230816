    ...
    class WorkloadTrendChart : TAP.Remoting.Server.Biz.BizComponentBase
    {
        ...
        public void GetWorkLoadTrendForInterval(AwrArgsPack arguments)
        {
            ...
            try
            {
                ...
                tmpSql.Append(@"WITH t1_sysmetric_summary AS
                            ...
                            ,SUM(DECODE(s.metric_name, 'Host CPU Utilization (%)', s.average, 0)) CPU_Util_pct
                            ,SUM(DECODE(s.metric_name, 'Host CPU Utilization (%)', s.maxval, 0)) CPU_Util_pct_max
                            ,SUM(DECODE(s.metric_name, 'Logical Reads Per Sec', s.average, 0)) Logical_Reads_psec
                            ,SUM(DECODE(s.metric_name, 'Physical Reads Per Sec', s.average, 0)) Physical_Reads_psec
                            ,SUM(DECODE(s.metric_name, 'Physical Writes Per Sec', s.average, 0)) Physical_Writes_psec
                            ,SUM(DECODE(s.metric_name, 'Executions Per Sec', s.average, 0)) Execs_psec_avg
                            ,SUM(DECODE(s.metric_name, 'Executions Per Sec', s.maxval, 0)) Execs_psec_max
                            ,SUM(DECODE(s.metric_name, 'User Calls Per Sec', s.average, 0)) User_Calls_psec
                            ,SUM(DECODE(s.metric_name, 'DB Block Changes Per Sec', s.average, 0)) DB_Block_Changes_psec
                            ,SUM(DECODE(s.metric_name, 'SQL Service Response Time', s.average, 0)) SQL_Service_Response_Time
                            ,SUM(DECODE(s.metric_name, 'User Commits Per Sec', s.average, 0)) User_Commits_psec
                            ,SUM(DECODE(s.metric_name, 'User Commits Per Sec', s.maxval, 0)) User_Commits_max_psec
                            ,SUM(DECODE(s.metric_name, 'Redo Generated Per Sec', s.average, 0)) Redo_Generated_psec
                            ,SUM(DECODE(s.metric_name, 'Redo Generated Per Sec', s.maxval, 0)) Redo_Generated_max_psec
                            ,SUM(DECODE(s.metric_name, 'Hard Parse Count Per Sec', s.maxval, 0)) HardParseCountPerSec
                            FROM (SELECT ");
                ...
                tmpSql.Append(@") s
                WHERE 1 = 1
                GROUP BY s.dbid, s.instance_number, s.snap_id), ");

                    tmpSql.Append(@"t1_sysstat AS(
                                   SELECT
                                   dbid
                                  ...
                                  ,( EXTRACT(HOUR FROM(cast(END_TIME as timestamp) - cast(BEGIN_TIME as timestamp))) * 60 * 60 +
                                           EXTRACT(MINUTE FROM(cast(END_TIME as timestamp) - cast(BEGIN_TIME as timestamp))) * 60 +
                                           EXTRACT(SECOND FROM(cast(END_TIME as timestamp) - cast(BEGIN_TIME as timestamp)))    )  snap_time
                ...
                tmpSql.AppendFormat("   FROM raw_DBA_HIST_SYSSTAT_{0} ss WHERE 1 = 1 AND ", arguments.DBName);
                tmpSql.AppendFormat("  ss.INSTANCE_NUMBER = {0}", arguments.INSTANCE_NUMBER);
                tmpSql.AppendFormat("   AND ss.dbid = {0}", arguments.DBID);
                tmpSql.Append(@"  AND TO_CHAR(ss.BEGIN_TIME, 'YYYYMMDD-HH24MI') BETWEEN ");
                tmpSql.AppendFormat("  '{0}-0000' AND '{1}-2400' ", arguments.StartTime,arguments.EndTime);
                tmpSql.Append(@")
                               WHERE 1 = 1
                               GROUP BY dbid, INSTANCE_NUMBER, snap_id) s),
                                ...
                                ss.end_time
                                from ");
                tmpSql.AppendFormat(@" raw_dba_hist_sqlstat_{0} ss ", arguments.DBName);
               tmpSql.AppendFormat(@" where to_char(ss.begin_time, 'yyyymmdd-hh24mi') between '{0}-0000' AND '{1}-2400' ", arguments.StartTime, arguments.EndTime);

                tmpSql.AppendFormat("   AND ss.INSTANCE_NUMBER = {0} ", arguments.INSTANCE_NUMBER);
                tmpSql.AppendFormat("   AND ss.dbid in ('{0}') and 1=1 )", arguments.DBID);
                tmpSql.Append(@"        group by dbid, inst_id,snap_id
                                    )  ");
        ...
        public void GetWorkLoadLagestDaySql(AwrArgsPack arguments)
        {
            ...
            try
            {
                ...
                tmpSql.Append("         WHERE T.begin_time > ");
                tmpSql.AppendFormat("               TO_DATE('{0}', 'yyyy-MM-dd HH24:mi:ss') ", arguments.StartTime);
                tmpSql.Append("           and t.begin_time <= ");
                tmpSql.AppendFormat("               TO_DATE('{0}', 'yyyy-MM-dd HH24:mi:ss') ", arguments.EndTime);
                tmpSql.AppendFormat("          ) b ");
                tmpSql.AppendFormat("          where {0} is not null", arguments.ParamNamesString);
                tmpSql.AppendFormat("            group by   sql_id,instance_number,b.module,b.action,b.parsing_schema_name order by {0} desc", arguments.ParamNamesString);
        ...
        public void GetWorkloadNaerTwoM(AwrArgsPack arguments)
        {
            ...
            {
                StringBuilder tmpSql = new StringBuilder();
                if (arguments.ParamNamesString == "ELAPSED_TIME_DELTA" || arguments.ParamNamesString == "CPU_TIME_DELTA"|| arguments.ParamNamesString== "CPU_UTIL_PCT" || arguments.ParamNamesString == "SQL_SERVICE_RESPONSE_TIME")
                {
                ...
                tmpSql.AppendFormat(" SELECT t.sql_id, TO_CHAR(t.begin_time, '{0}') workDate, T.{1},t.instance_number,t.module,t.action,t.parsing_schema_name", arguments.PARADEF, arguments.ParamNamesString);
                tmpSql.AppendFormat(" FROM raw_dba_hist_sqlstat_{0} T ", arguments.DBName);
                tmpSql.AppendFormat(" WHERE t.begin_time >= TO_DATE('{0}', 'yyyy-MM-dd HH24:mi:ss')  and  t.begin_time <= TO_DATE('{1}', 'yyyy-MM-dd HH24:mi:ss')  ", arguments.StartTime, arguments.EndTime);
                tmpSql.AppendFormat("AND t.sql_id = '{0}' ", arguments.ParamType);
                tmpSql.Append(" ORDER BY T.begin_time) d ");
                tmpSql.AppendFormat("where d.instance_number = '{0}' ", arguments.INSTANCE_NUMBER);
                tmpSql.Append(" GROUP BY d.workdate,d.sql_id,d.instance_number,d.module,d.action,d.parsing_schema_name ORDER BY d.workdate");
                RemotingLog.Instance.WriteServerLog(MethodInfo.GetCurrentMethod().Name, LogBase._LOGTYPE_TRACE_INFO, this.Requester.IP,
                       tmpSql.ToString(), false);
