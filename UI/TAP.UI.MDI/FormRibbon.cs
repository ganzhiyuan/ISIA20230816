...
namespace TAP.UI.MDI
{
    ...
    public partial class FormRibbon : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        public FormRibbon(string appName, string mdiName)
        {
            InitializeComponent();
        ...
        private void LoadUserGroupInformation()
        {
            ...
            try
            {
                ...
                tmpUserDefaultInfo.UserGroup = InfoBase._USER_INFO.UserGroupName;
        ...
        private RibbonPageGroup CreateMenuBarItemRibbon(MainMenuBasicModel mainMenuModel)
        {
            ...
            try
            {
                ...
                //temp start
                BarSubItem tmpSubMonitoring = new BarSubItem();
                BarSubItem tmpSubAnalysis = new BarSubItem();
                BarSubItem tmpSubSpec = new BarSubItem();
                BarSubItem tmpSubReport = new BarSubItem();

                if (mainMenuModel.Name == "ANALYSIS")
                {
                    
                    tmpSubMonitoring.Name = "Monitoring";
                    tmpSubMonitoring.Caption = this._conveter.ConvertPhrase("MONITORING");
                    //tmpSubMonitoring.Tag = tmpUIModel.Name;
                    tmpSubMonitoring.AllowRightClickInMenu = false;
                    tmpSubMonitoring.Alignment = BarItemLinkAlignment.Default;
                    tmpSubMonitoring.ImageOptions.SvgImage = DevExpress.Utils.Svg.SvgImage.FromFile(System.IO.Path.Combine(this._appDirectory, "IMAGES", "MENU", "BIG", "Monitoring.svg"));
                    
                    tmpSubAnalysis.Name = "Analysis";
                    tmpSubAnalysis.Caption = this._conveter.ConvertPhrase("ANALYSIS");
                    //tmpSubAnalysis.Tag = tmpUIModel.Name;
                    tmpSubAnalysis.AllowRightClickInMenu = false;
                    tmpSubAnalysis.Alignment = BarItemLinkAlignment.Default;
                    tmpSubAnalysis.ImageOptions.SvgImage = DevExpress.Utils.Svg.SvgImage.FromFile(System.IO.Path.Combine(this._appDirectory, "IMAGES", "MENU", "BIG", "Analysis.svg"));
                                        
                    tmpSubSpec.Name = "SPEC";
                    tmpSubSpec.Caption = this._conveter.ConvertPhrase("SPEC");
                    //tmpSubSpec.Tag = tmpUIModel.Name;
                    tmpSubSpec.AllowRightClickInMenu = false;
                    tmpSubSpec.Alignment = BarItemLinkAlignment.Default;
                    tmpSubSpec.ImageOptions.Image = global::TAP.UI.MDI.Properties.Resources.chartyaxissettings_32x32;
                    tmpSubSpec.ImageOptions.LargeImage = global::TAP.UI.MDI.Properties.Resources.chartyaxissettings_32x32;

                    tmpSubReport.Name = "Report";
                    tmpSubReport.Caption = this._conveter.ConvertPhrase("REPORT");
                    //tmpSubMonitoring.Tag = tmpUIModel.Name;
                    tmpSubReport.AllowRightClickInMenu = false;
                    tmpSubReport.Alignment = BarItemLinkAlignment.Default;
                    tmpSubReport.ImageOptions.SvgImage = DevExpress.Utils.Svg.SvgImage.FromFile(System.IO.Path.Combine(this._appDirectory, "IMAGES", "MENU", "BIG", "report.svg"));
                    
                    tmpRibbonPageGroup.ItemLinks.Add(tmpSubMonitoring);
                    tmpRibbonPageGroup.ItemLinks.Add(tmpSubAnalysis);
                    tmpRibbonPageGroup.ItemLinks.Add(tmpSubSpec);
                    tmpRibbonPageGroup.ItemLinks.Add(tmpSubReport);
                    this.ribbonControl1.Items.Add(tmpSubMonitoring);
                    this.ribbonControl1.Items.Add(tmpSubAnalysis);
                    this.ribbonControl1.Items.Add(tmpSubSpec);
                    this.ribbonControl1.Items.Add(tmpSubReport);

                    tmpSubAnalysis.ItemClick += new ItemClickEventHandler(barSubItem1_ItemClick);
                    tmpSubMonitoring.ItemClick += new ItemClickEventHandler(barSubItem1_ItemClick);
                    tmpSubSpec.ItemClick += new ItemClickEventHandler(barSubItem1_ItemClick);
                    tmpSubReport.ItemClick += new ItemClickEventHandler(barSubItem1_ItemClick);
                    ...
                    if (this.CheckUserAuthority(tmpUIModel))
                    {
                        ...
                        if (tmpUIModel.Name.IndexOf("SPREATOR") < 0)
                        {
                            ...
                            if (mainMenuModel.Name == "ANALYSIS")
                            {
                                ...
                                string tmpstr = tmpNewButton.Tag.ToString();

                                if (tmpstr.IndexOf("SPEC") != -1)
                                {
                                    tmpSubSpec.ItemLinks.Add(tmpNewButton);
                                }
                                else if (tmpstr.IndexOf("TRENDCHART") != -1 || tmpstr.IndexOf("RESPONSEMYCHART") != -1)
                                {
                                    tmpSubMonitoring.ItemLinks.Add(tmpNewButton);
                                }
                                else if (tmpstr.IndexOf("ANALYSIS") != -1)
                                {
                                    tmpSubAnalysis.ItemLinks.Add(tmpNewButton);
                                }
                                else
                                {
                                    tmpSubReport.ItemLinks.Add(tmpNewButton);
                                }
