...
namespace TAP.UI.MDI
{
    ...
    public partial class Form1 : XtraForm
    {
        ...
        public Form1(string appName, string mdiName)
        {
            InitializeComponent();
        ...
        protected void TimerStart(object sender, EventArgs e)
        {
            ...
            try
            {
                string time = DateTime.Now.ToString(); //basic Type.. "yyyy-MM-dd HH:mm:ss"
                this.barDate.Caption = time;
            }
            catch (System.Exception ex)
            {
                throw ex;
            ...
            try
            {
                ...
                for (int i = 0; i < tmpMainMenuNames.Length; i++)
                {
                    ...
                    if (_intSpreator.Length > 0)
                    {
                        ...
                        {
                            if(tmpBarMenu.ItemLinks.Count > j)
                                tmpBarMenu.ItemLinks[j].BeginGroup = true;
                        }
                    }


                    //The menu will not be displayed without the UI.
                    if (tmpBarMenu.ItemLinks.Count > 0)
                        this.barMenu.AddItem(tmpBarMenu);
                    //seoil end
        ...
        private void InitializeXtraBar()
        {
            ...
            try
            {
                ...
                if (TAP.Base.Configuration.ConfigurationManager.Instance.AppSection.MDIName == "RMS")
                {
                    ...
                    barSite.Caption = barSite.Caption + InfoBase._USER_INFO.Region.ToString();
                    barFactory.Caption = barFactory.Caption + InfoBase._USER_INFO.Facility.ToString();
                    barUser.Caption = barUser.Caption + InfoBase._USER_INFO.Name.ToString();
                    LoadTime(); //barDate Time Update..
                }
                else
                {
        ...
        private bool CloseApp()
        {
            ...
            try
            {
                //자신의 정보를 수정하였을 경우 다시 가져올수 있도록 처리함.
                InfoBase._USER_INFO = new UserModel(this._userDefaultInfo, null, EnumFlagYN.YES);
